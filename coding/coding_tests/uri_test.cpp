...
{
  char const uriS[] = "mapsrahal://map?ll=10.3,12.3223&n=Hello%20World";
  Uri uri(uriS, ARRAY_SIZE(uriS) - 1);
  TEST_EQUAL(uri.GetScheme(), "mapsrahal", ());
}

UNIT_TEST(UriInvalidSchemeNoColon)
{
  TEST_EQUAL(Uri("mapsrahal:").GetScheme(), "mapsrahal", ());
}

UNIT_TEST(UriTestValidScheme2)
{
  TestUri("mapsrahal://map?ll=10.3,12.3223&n=Hello%20World")
      .Scheme("mapsrahal")
      .Path("map")
      .KV("ll", "10.3,12.3223")
      .KV("n", "Hello World");
...
UNIT_TEST(UriComprehensive)
{
  ...
  TestUri("mapsrahal://map?ll=1.2,3.4&z=15").Scheme("mapsrahal").Path("map")
