...
{
  string uriString = "mapsrahal://map?ll=38.970559,-9.419289&ignoreThisParam=Yes&z=17&n=Point%20Name";
...
UNIT_TEST(RouteApiSmoke)
{
  string const uriString =
      "mapsrahal://route?sll=1,1&saddr=name0&dll=2,2&daddr=name1&type=vehicle";
  TEST(Uri(uriString).IsValid(), ());
...
UNIT_TEST(MapApiInvalidUrl)
{
  ...
  TEST(!IsValid(fm, "mapsrahal://ggg?ll=12.3,34.54"), ());
  TEST(!IsValid(fm, "mwm://"), ("No parameters"));
  TEST(!IsValid(fm, "mapsrahal://map?"), ("No longtitude"));
  TEST(!IsValid(fm, "mapsrahal://map?ll=1,2,3"), ("Too many values for ll"));
  TEST(!IsValid(fm, "mapsrahal://fffff://map?ll=1,2"), ());
...
{
  Framework f(kFrameworkParams);
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&saddr=name0&dll=2,2&daddr=name2"),
       ("Route type doesn't exist"));
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&saddr=name0"), ("Destination doesn't exist"));
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&dll=2,2&type=vehicle"),
       ("Source or destination name doesn't exist"));
  TEST(!IsValid(f, "mapsrahal://route?saddr=name0&daddr=name1&type=vehicle"), ());
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&sll=2.2&type=vehicle"), ());
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&dll=2.2&type=666"), ());
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&saddr=name0&sll=2,2&saddr=name1&type=vehicle"), ());
  TEST(!IsValid(f, "mapsrahal://route?sll=1,1&type=vehicle"), ());
  TEST(!IsValid(f,
                "mapsrahal://"
                "route?sll=1,1&saddr=name0&sll=2,2&saddr=name1&sll=1,1&saddr=name0&type=vehicle"),
       ());
  TEST(!IsValid(f, "mapsrahal://route?type=vehicle"), ());
}

UNIT_TEST(MapApiLatLonLimits)
{
  Framework fm(kFrameworkParams);
  TEST(!IsValid(fm, "mapsrahal://map?ll=-91,10"), ("Invalid latitude"));
  TEST(!IsValid(fm, "mwm://map?ll=523.55,10"), ("Invalid latitude"));
  TEST(!IsValid(fm, "mapsrahal://map?ll=23.55,450"), ("Invalid longtitude"));
  TEST(!IsValid(fm, "mapsrahal://map?ll=23.55,-450"), ("Invalid longtitude"));
}

UNIT_TEST(MapApiPointNameBeforeLatLon)
{
  ApiTest test("mapsrahal://map?n=Name&ll=1,2");
  TEST(!test.IsValid(), ());
...
{
  {
    ApiTest api("mapsrahal://map?ll=1,2&n=A&N=B");
    TEST(api.IsValid(), ());
...
{
  ApiTest api("mapsrahal://map?ll=1,1,1&n=A&ll=2,2&n=B&ll=3,3,3&n=C");
  TEST(!api.IsValid(), ());
...
UNIT_TEST(GlobalBackUrl)
{
  ...
  {
    ApiTest api("mwm://map?ll=1,2&n=PointName&backurl=http://mapsrahal.com");
    TEST_EQUAL(api.GetGlobalBackUrl(), "http://mapsrahal.com", ());
  }
  {
  ...
  {
    ApiTest api("mwm://map?ll=1,2&n=PointName&backurl=%E6%88%91%E6%84%9Bmapsrahal");
    TEST_EQUAL(api.GetGlobalBackUrl(), "我愛mapsrahal://", ());
  }
}
...
void generateRandomTest(uint32_t numberOfPoints, size_t stringLength)
{
  ...
  string result = "mapsrahal://map?v=1";
...
{
  {
    ApiTest api("mapsrahal://map?ll=38.970559,-9.419289&ignoreThisParam=Yes&z=17&n=Point%20Name");
    TEST(!api.GoBackOnBalloonClick(), (""));
  }
  {
    ApiTest api("mapsrahal://map?ll=38.970559,-9.419289&ignoreThisParam=Yes&z=17&n=Point%20Name&balloonaction=false");
    TEST(api.GoBackOnBalloonClick(), (""));
  }
  {
    ApiTest api("mapsrahal://map?ll=38.970559,-9.419289&ignoreThisParam=Yes&z=17&n=Point%20Name&balloonaction=true");
    TEST(api.GoBackOnBalloonClick(), (""));
  }
  {
    ApiTest api("mapsrahal://map?ll=38.970559,-9.419289&ignoreThisParam=Yes&z=17&n=Point%20Name&balloonaction=");
    TEST(api.GoBackOnBalloonClick(), (""));
  }
}
